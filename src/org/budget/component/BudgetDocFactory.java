...
import org.idempiere.budget.I_B_BudgetPeriod;

public class BudgetDocFactory  implements IDocFactory{

	private final static CLogger	s_log	= CLogger.getCLogger(BudgetDocFactory.class);
	...
	public Doc getDocument(MAcctSchema as, int AD_Table_ID, int Record_ID, String trxName)
	{
		...
		if (tableName.equals(I_B_BudgetPeriod.Table_Name))
		{
			Doc doc = null;
	...
	public Doc getDocument(MAcctSchema as, int AD_Table_ID, ResultSet rs, String trxName)
	{
		...
		if (tableName.equals(I_B_BudgetPeriod.Table_Name))
		{
			return new Doc_BudgetPeriod(as, rs, trxName);
		}
		return null;
	}
